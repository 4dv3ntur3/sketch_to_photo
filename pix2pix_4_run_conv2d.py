...
def decoder_block(layer_in, skip_in, n_filters, dropout=True):
	...
	# layer_ix = list(skip_in)
	# layer_ix = layer_ix[1]-7

	# l_size = np.array([1, 2, 4, 8, 16, 32, 64, 128, 256])

	# g = Reshape((l_size[layer_ix], l_size[layer_ix], n_filters))(layer_in)
...
def define_generator(image_shape=(256,256,3)):
	...
	e9 = Activation('relu')(e8)
	# decoder model

	d1 = decoder_block(e9, e7, 512)
	...
	g = Reshape((256, 256, 32))(d7) #image shape[1], image_shape[2], concatenate한 encoder layer의 n_filters
...
# layer output 
# print(d_model.layers)

# print(d_model.layers[0].output_shape)

# layer_shape = []
# for i in range(8):
#     layer_shape.append(np.array(d_model.layers[i].output_shape))
